...
// `db` is loaded once from MongoDB at startup (see the bottom of this file)
// and mutated in memory exactly like before; every route that changes it
// now awaits save(db) to persist that change back to MongoDB.
let db;
...
app.patch("/api/participants/:id/progress", async (req, res) => {
...
/* ---------------------------------- START ---------------------------------- */
// Data is loaded from MongoDB before the server starts accepting requests,
// so no route ever runs against an empty/undefined `db`.
